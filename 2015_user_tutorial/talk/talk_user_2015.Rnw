...
\title{Applied Machine Learning and Efficient Model Selection with \texttt{mlr}}
...
\begin{vframe}{Welcome!}
  \begin{itemize}
    \item Project home page\\\url{https://github.com/berndbischl/mlr}
      \begin{itemize}
        \item R documentation rendered in HTML
        \item Tutorial for online viewing / download, including many examples
        \item Don't hesitate to interrupt us
        \item There will be a coffee break
        % \item Wiki page for this tutorial (slides, hands on solutions, \ldots)
      \end{itemize}
    % \item 6 main developers, quite a few contributors, 3 GSOC projects
...
\begin{vframe}{Abstractions for Machine Learning}
  \begin{center}
    \includegraphics[width=0.85\textwidth]{figure/ml_abstraction-crop.pdf}
  \end{center}
  \begin{itemize}
    \item All learning tasks fit well into this scheme
    \item Challenge as statistician: find a suitable model to maximize the outcome (or minimize the loss)
  \end{itemize}
\end{vframe}
...
\begin{vframe}{Visualizations}
  \begin{itemize}
    ...
    \item Current GSOC project with Zach Jones
    \begin{itemize}
...
\begin{vbframe}{caret vs. mlr}
  ...
  %FIXME we need to be really careful here and check this!
